...
export default class OwnPostsScreen extends Component {
    ...
    updateContent() {
        this.profileRef.doc(this.userId).get().then(doc => {
            if (doc.exists) {
                let itemsToGet = Object.keys(doc.data().own_posts);
    ...
    deletePost = post => {
        var post_id = post.id;
        Fire.shared.deleteOwnPostFromMap(post_id);
        Fire.shared.deleteOwnPostMarker(post_id);
        this.removeFromArray(post_id)

        var image = post.image_ref;
        console.log(image)
        ...
        if(image === 'no image'){
            ...
            console.log('image is present at '+image)
